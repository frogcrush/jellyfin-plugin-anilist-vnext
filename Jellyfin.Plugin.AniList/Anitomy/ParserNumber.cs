...
namespace AnitomySharp
{
  ...
  public class ParserNumber
  {
    ...
    private static bool IsValidEpisodeNumber(string number)
    {
      ...
      return !string.IsNullOrEmpty(temp) && double.Parse(temp, CultureInfo.InvariantCulture) <= EpisodeNumberMax;
    }
    ...
    public bool SearchForLastNumber(List<int> tokens)
    {
      for (var i = tokens.Count - 1; i >= 0; i--)
      {
        ...
        if (_parser.ParseHelper.IsTokenCategory(previousToken, Token.TokenCategory.Unknown))
        {
          if (_parser.Tokens[previousToken].Content.Equals("Movie", StringComparison.OrdinalIgnoreCase) 
              || _parser.Tokens[previousToken].Content.Equals("Part", StringComparison.OrdinalIgnoreCase))
          {
            continue;
          }
        }
