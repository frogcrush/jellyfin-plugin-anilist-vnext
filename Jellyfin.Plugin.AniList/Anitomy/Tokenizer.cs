  ...
  public class Tokenizer
  {
    ...
    private string GetDelimiters(TokenRange range)
    {
      ...
      bool IsDelimiter(char c)
      {
        ...
        return _options.AllowedDelimiters.Contains(c.ToString(), StringComparison.Ordinal) && !delimiters.ToString().Contains(c.ToString(), StringComparison.Ordinal);
      }
    ...
    private void TokenizeByDelimiters(bool enclosed, TokenRange range)
    {
      ...
      for (int i = range.Offset, end = range.Offset + range.Size; i < end;)
      {
        var found = Enumerable.Range(i, Math.Min(end, _filename.Length) - i)
          .Where(c => delimiters.Contains(_filename[c].ToString(), StringComparison.Ordinal))
          .DefaultIfEmpty(end)
          .FirstOrDefault();
